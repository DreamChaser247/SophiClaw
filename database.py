...
SEED_TOPICS = [
    ...
    ("FUNK_TRYG",    "Funkcje trygonometryczne",     "FUNK",      2, "sin, cos, tan, cot — definicje, wykresy, jedynka"),
    ...
    ("GEOM_PLAN",    "Planimetria",                  "GEOM",      2, "Trójkąty, czworokąty, okrąg, Pitagoras"),
    ...
    ("RACHPRAW_STAT","Statystyka",                   "RACHPRAW",  2, "Średnia, mediana, odchylenie, dane"),
...
class Database:
    ...
    def _seed_topics(self) -> None:
        if self.conn.execute("SELECT COUNT(*) FROM topics").fetchone()[0] > 0:
            return
    ...
    def add_attempt(
        ...
        self.conn.execute(
            """INSERT INTO attempts
               (session_id, task_id, topic_code, score, difficulty, llm_json, is_correct)
               VALUES (?,?,?,?,?,?,?)""",
            (session_id, task_id, topic_code, score, difficulty, llm_json, 1 if score >= 4 else 0),
        )
    ...
    def _update_mastery(self, topic_code: str) -> None:
        row = self.conn.execute(
            "SELECT AVG(CAST(score AS REAL)/6*100) as avg FROM attempts WHERE topic_code=?",
            (topic_code,),
        ).fetchone()
    ...
    def get_session_attempts(self, session_id: str) -> list:
        rows = self.conn.execute(
            "SELECT topic_code as topic, difficulty, score FROM attempts WHERE session_id=? ORDER BY timestamp",
            (session_id,),
        ).fetchall()
    ...
    def get_last_attempts(self, limit: int = 10) -> list:
        rows = self.conn.execute(
            """SELECT a.timestamp, a.topic_code, t.name as topic_name, a.score, a.difficulty
               FROM attempts a
    ...
    def add_note(self, session_id: str, content: str, topic_code: Optional[str] = None) -> None:
        self.conn.execute(
            "INSERT INTO notes (session_id, topic_code, content) VALUES (?,?,?)",
            (session_id, topic_code, content[:500]),
        )
        self.conn.commit()

    def get_session_notes(self, session_id: str) -> list:
        rows = self.conn.execute(
            "SELECT content FROM notes WHERE session_id=? ORDER BY created_at",
            (session_id,),
        ).fetchall()
    ...
    def get_summary_stats(self) -> dict:
        ...
        stats["weakest_topics"] = [
            dict(r) for r in self.conn.execute(
                "SELECT name, mastery FROM topics WHERE mastery > 0 ORDER BY mastery ASC LIMIT 3"
            ).fetchall()
        ]
        stats["strongest_topics"] = [
            dict(r) for r in self.conn.execute(
                "SELECT name, mastery FROM topics WHERE mastery > 0 ORDER BY mastery DESC LIMIT 3"
            ).fetchall()
        ]
